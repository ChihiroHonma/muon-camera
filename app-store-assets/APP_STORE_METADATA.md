# ZERO Camera — App Store登録内容

## 基本情報

- アプリ名：`ZERO Camera`
- サブタイトル：`マナーモードに合わせて静かに撮影`
- プライマリカテゴリ：写真／ビデオ
- セカンダリカテゴリ：ユーティリティ
- 著作権：`2026 カナエル`
- サポートURL：`https://kanaeru-apps.github.io/support.html`
- プライバシーポリシーURL：`https://kanaeru-apps.github.io/privacy-policy.html`

## 説明文

ZERO Cameraは、端末のマナーモードに合わせて静かに撮影できる、シンプルなカメラアプリです。

マナーモード中は静かに写真を撮影でき、通常モードでは撮影音で撮影したことをお知らせします。写真だけでなく、音声付き動画の撮影にも対応しています。

撮影した写真や動画は端末内で処理され、外部サーバーへ送信されません。必要な写真・動画だけを写真ライブラリへ保存したり、iOSの共有機能を使ってほかのアプリへ送ったりできます。

主な機能
・写真撮影
・音声付き動画撮影
・マナーモードに連動した撮影音
・前面／背面カメラの切り替え
・ズーム調整
・明るさ調整
・1:1、3:4、9:16、全画面の画角切り替え
・撮影タイマー
・グリッド表示
・フラッシュ切り替え
・写真ライブラリへの保存
・iOS標準の共有機能

会員登録やログインは必要ありません。広告やアクセス解析機能も使用していません。

※動画撮影では音声を記録するため、マイクへのアクセスが必要です。
※撮影音の動作は端末の設定や動作環境によって異なる場合があります。

## キーワード（93バイト）

`カメラ,マナー,静音,写真,動画,撮影,ズーム,タイマー,グリッド,明るさ`

## App Review Notes

This is a camera application supporting photo and video capture. No login is required.

Photo capture follows the iPhone's Silent Mode setting. When Silent Mode is enabled, photos are captured quietly. When Silent Mode is disabled, the app plays a shutter sound to notify the user that a photo was taken.

Camera permission is used for photo and video capture. Microphone permission is used only to record audio with videos. Photo Library Add permission is used only when the user selects Save. Captured media is processed on the device and is not uploaded to the developer's server.

To test:
1. Allow Camera, Microphone, and Add Photos permissions.
2. Enable or disable Silent Mode and take a photo.
3. Switch to Video, record a short video, and confirm that it contains audio.
4. Select Save to add the captured media to the photo library.

## 申告方針

- App Privacy：このアプリからデータを収集しない
- 年齢制限：各コンテンツ項目なし（想定4+）
- Made for Kids：選択しない
- 非免除暗号化：なし（`ITSAppUsesNonExemptEncryption=false`）
- リリース方法：審査通過後に手動でリリース
