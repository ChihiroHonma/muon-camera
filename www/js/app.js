...
//
// disableAudio: false (マイクを常時セッションに保持) は絶対に変えないこと。
// マイク入力があるとiOSは「録音コンテキスト」とみなし、日本向けiPhoneの
// OS強制シャッター音を抑制する。これが本アプリの無音撮影の土台。
// (実機検証: マイクを外した途端、OS標準のシャッター音が復活して常時鳴った)
// シャッター音のマナー連動はShutterSoundPlugin側のシステムサウンドAPIが担う。
function nativeStartOptions() {
  return {
    ...
    // 録画中プレビュー暗転は patch-package のプラグイン修正で対処
...
document.querySelectorAll('.mode-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (useNativeCam) {
      ...
      // ※モード切替でのカメラ再起動はしない（disableAudioは常時false固定のため不要。
      //   再起動方式は実機で動画モードを壊したため撤回済み）。
